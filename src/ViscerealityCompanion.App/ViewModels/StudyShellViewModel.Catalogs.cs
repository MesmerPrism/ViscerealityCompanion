...
public sealed partial class StudyShellViewModel
{
    ...
    [
        ...
        new(6, "Review the device profile", "Apply the pinned CPU, GPU, brightness, and media-volume profile, then review the battery and remaining bench advisories before leaving the bench. These checks stay visible, but they do not block the study flow."),
        ...
        new(8, "Launch the runtime", "Wake the headset to enable launching, then launch the study runtime. On the current Meta OS build the launch path still applies best-effort task pinning, but the controller Meta/menu button may remain active."),
        new(9, "Verify LSL reaches the headset", "Confirm the external heartbeat/biofeedback LSL stream is reaching the study runtime and that the resulting live state is visible back in the companion."),
        new(10, "Test particle commands", "Use the companion controls to turn particles on and then off again. This is still recommended for bench confidence, but it does not block the study flow."),
